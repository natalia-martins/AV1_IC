...
class MultiDinoGame:
    ...
    def step(self, actions):
        ...
        for player in self.player_dinos:
            if player.is_dead:
                continue

            if len(self.cacti) > 0:
                cactus = self.cacti.sprites()[0]
                distance_to_cactus = cactus.rect.left - player.rect.right
                if distance_to_cactus < 100 and player.rect.bottom == int(0.98 * HEIGHT) and player.jump_cooldown == 0:
                    actions[0] = ACTION_UP

            if len(self.pteras) > 0:
                ptera = self.pteras.sprites()[0]
                if ptera.rect.centery < HEIGHT * 0.75 and player.rect.right + 50 >= ptera.rect.left:
        ...
        for player in self.alive_players:
            player.draw()
        if len(self.alive_players) == 0 and self.last_dead_player:
